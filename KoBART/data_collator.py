...
class DataCollatorForTextInfilling:
    ...
    def mask_tokens(self, inputs):
        ...
        # batching now fixed
        return torch.tensor(new_inputs.tolist()), torch.tensor(labels.tolist())
...
class DataCollatorForSentencePermutation:
    ...
    def __call__(self, example: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        example["decoder_input_ids"] = torch.tensor(example["input_ids"])
        example["input_ids"] = torch.tensor(result)
